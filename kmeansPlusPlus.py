...
def calculateDistanceKpp(centroid, dataPoint):
    array1 = np.array(centroid[1][:-1])
    array2 = np.array(dataPoint[1][:-1])
    print(array1)
    print(array2)
...
def distanceFromCentroids(data, centroids):
    D2 = np.array([min([calculateDistanceKpp(centroid, dataPoint) for centroid in centroids]) for dataPoint in data])
    return D2
...
def chooseNextCentroid(D2, dataArray):
    ...
    centroidIndex = np.where(cumprobs >= randomValue)[0][0]
...
if len(sys.argv) != 4:
    ...
    print("Try executing the following command : spark-submit kmeans.py data/iris_clustering.dat 3 20")
    exit(0)
